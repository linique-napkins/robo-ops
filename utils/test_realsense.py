...
def get_device_info(device: rs.device) -> dict[str, str]:
    ...
    fields = [getattr(rs.camera_info, name) for name in labels]
...
def draw_text(
    ...
    for i, line in enumerate(lines):
        ...
        if bg:
            ...
            p1 = (pos[0] - 2, pos[1] - th - 4)
            p2 = (pos[0] + tw + 2, pos[1] + 4)
...
def has_imu(device: rs.device) -> bool:
    ...
    return any(sensor.is_motion_sensor() for sensor in device.query_sensors())


def find_device(serial_filter: str | None) -> rs.device:
    """Discover RealSense devices and return the target device."""
    ctx = rs.context()
    ...
    for dev in devices:
        ...
        print(f"  [{sn}] {name}")
        if serial_filter is None or sn == serial_filter:
            target_device = dev

    if target_device is None:
        print(f"\nERROR: No device found with serial '{serial_filter}'")
        sys.exit(1)
...
def setup_pipeline(
    serial: str,
    target_device: rs.device,
    enable_imu: bool,
) -> tuple[rs.pipeline, rs.pipeline_profile, bool]:
    ...
    imu_enabled = False
    if has_imu(target_device) and enable_imu:
        try:
            test_cfg = rs.config()
    ...
    return pipeline, profile, imu_enabled


def print_diagnostics(
    profile: rs.pipeline_profile,
    imu_enabled: bool,
) -> tuple[float, rs.colorizer]:
    """Print active stream info, intrinsics, and depth sensor options."""
    ...
    print(
        ...
        f"@ {color_profile.fps()}fps ({color_profile.format().name})"
    )
    print(
        f"  Depth: {depth_profile.width()}x{depth_profile.height()} "
        f"@ {depth_profile.fps()}fps ({depth_profile.format().name})"
    )
    print(f"  Depth scale: {depth_scale:.6f} m/unit")
    if imu_enabled:
        print("  IMU: accelerometer + gyroscope enabled")

    print("\n=== Intrinsics ===")
    print(
        f"  Color: fx={color_intrinsics.fx:.1f} fy={color_intrinsics.fy:.1f} "
        f"cx={color_intrinsics.ppx:.1f} cy={color_intrinsics.ppy:.1f} "
        f"model={color_intrinsics.model}"
    )
    print(
        f"  Depth: fx={depth_intrinsics.fx:.1f} fy={depth_intrinsics.fy:.1f} "
    ...
    return depth_scale, rs.colorizer()
...
def stream_loop(
    ...
    """Run the main frame capture and display loop."""
    frame_count = 0
    t_start = time.time()
    fps_display = 0.0
    ...
    try:
        while True:
            ...
            d_min = valid.min() if len(valid) > 0 else 0.0
            d_max = valid.max() if len(valid) > 0 else 0.0
            d_mean = valid.mean() if len(valid) > 0 else 0.0
            ...
            if imu_enabled:
                overlay_lines += [
                    f"Accel: x={accel_data[0]:+.2f} y={accel_data[1]:+.2f}"
                    f" z={accel_data[2]:+.2f} m/s2",
                    f"Gyro:  x={gyro_data[0]:+.3f} y={gyro_data[1]:+.3f}"
                    f" z={gyro_data[2]:+.3f} rad/s",
                ]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Device serial number (uses first device if omitted)",
    )
    parser.add_argument(
        "--imu",
        action="store_true",
        help="Enable IMU streams (needs udev rules)",
    )
    args = parser.parse_args()

    target_device = find_device(args.serial)
    ...
    for s in sensor_info:
        ...
        for p in (p for p in s["profiles"] if p["default"]):
            print(f"    {p['stream']} {p['w']}x{p['h']} @ {p['fps']}fps ({p['format']})")

    usb_type = dev_info.get("usb_type_descriptor", "3")
    if usb_type.startswith("2"):
        print(f"\nERROR: RealSense on USB {usb_type} — requires USB 3.0+.")
        sys.exit(1)

    pipeline, profile, imu_enabled = setup_pipeline(serial, target_device, args.imu)
    depth_scale, colorizer = print_diagnostics(profile, imu_enabled)
    ...
    stream_loop(pipeline, colorizer, depth_scale, info_lines, imu_enabled)
